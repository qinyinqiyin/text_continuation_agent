...
def upload_article():
    ...
    try:
        ...
        # 如果段落数量较多，强制分段（避免合并成一个大段落）
        force_split_threshold = max(2, len(paragraphs) // 3)  # 每3个段落或每2个段落分段一次
        ...
        for i, para in enumerate(paragraphs):
            ...
            if len(para_text) > chunk_size:
                ...
                # 强制分段策略：
                # 1. 超过chunk_size
                # 2. 每2-3个段落分段一次（即使总长度小于chunk_size）
                # 3. 当前块长度超过chunk_size的70%
                should_split = (would_exceed or 
                               (para_count >= force_split_threshold and current_chunk) or
                               (len(current_chunk) > chunk_size * 0.7 and para_count >= 2))
        ...
        if len(segments) == 1:
            ...
            # 如果单个段落超过chunk_size，或者原始有多个段落，强制切分
            if len(single_seg) > chunk_size or len(paragraphs) > 1:
                logger.info(f"检测到只有1个分段但原始有{len(paragraphs)}个段落，长度={len(single_seg)}，强制按chunk_size={chunk_size}切分")
                segments = []
                
                # 策略1：如果有多个原始段落，直接按原始段落分段（每2-3个一段）
                if len(paragraphs) > 1:
                    logger.info(f"按原始段落重新分段，共{len(paragraphs)}个段落")
                    temp_chunk = ""
                    para_in_chunk = 0
                    for para in paragraphs:
                        ...
                        if not para:
                            continue
                        if para_in_chunk >= 2 or (temp_chunk and len(temp_chunk) + len(para) + 2 > chunk_size):
                            if temp_chunk.strip():
                                segments.append(temp_chunk.strip())
                            temp_chunk = para
                            para_in_chunk = 1
                ...
                if len(segments) == 1:
                    ...
                    # 如果按句号切分后还是只有1段，尝试更激进的切分
                    if len(segments) == 1 and len(segments[0]) > chunk_size * 0.5:
                        logger.info("按句号切分后仍只有1段，尝试按逗号切分")
                        single_seg = segments[0]
                        segments = []
                        parts = re.split(r'[，；]', single_seg)
                        temp_chunk = ""
                        for part in parts:
                            part = part.strip()
                            if not part:
